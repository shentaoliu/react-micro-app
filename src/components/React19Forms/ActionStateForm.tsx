...
const submitAction = async (prevState: any, formData: FormData) => {
  try {
    ...
    return { success: false, errorMsg: error.message, username: formData.get("username") as string };
  }
};
...
const ActionStateForm: React.FC = () => {
  ...
  return (
    <Card title="3. 进阶示例：使用 useActionState 管理状态" style={{ marginBottom: "24px" }}>
      <Paragraph>
        在基础示例中，我们还需要手动写 <code>useState</code> 来管理 <code>isLoading</code> 和错误信息。
        React 19 提供了 <strong><code>useActionState</code></strong>，它可以帮我们把表单的<strong>提交结果(state)</strong>和<strong>挂起状态(isPending)</strong>全部接管！
      </Paragraph>
      ...
      <form action={formAction} style={{ maxWidth: '300px' }}>
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <div>
            ...
            <Input name="username" defaultValue={state.username} placeholder="请输入 admin" />
          </div>
          <div>
            <label>密码：</label>
